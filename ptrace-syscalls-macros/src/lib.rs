...
impl Decoder {
    fn decoded(
        ...
        if !is_result {
            ...
            quote_spanned! {
              span =>
              let #arg_name = if let Some(&#tmp_name) = (#args).as_ref().ok() {
...
fn gen_syscall_args_struct(
    ...
    for arg in args.iter() {
        ...
        let (wrapped_arg_type, need_memory_inspection) =
            wrap_syscall_arg_type(arg_type, crate_token.clone(), span);
        ...
        if !need_memory_inspection {
            ...
            if let Some(decoder) = &arg.decoder {
    ...
    if let Some(modified_args) = &syscall.modified_args {
        for modified_arg in modified_args.args.iter() {
            ...
            if !need_memory_inspection {
                ...
                if let Some(decoder) = &modified_arg.decoder {
                    let decoded = decoder.decoded(arg_name, wrapped_arg_type, span);
    ...
    match ty {
        ...
        Type::Path(ty) => {
            ...
            match ty_str.as_str() {
                ...
                "RawFd" | "socklen_t" | "c_int" | "c_uint" | "c_ulong" | "c_long" | "i16"
                | "i32" | "i64" | "u64" | "usize" | "isize" | "size_t" | "key_serial_t"
                | "AddressType" | "mode_t" | "uid_t" | "pid_t" | "gid_t" | "off_t" | "u32"
                | "clockid_t" | "id_t" | "key_t" | "mqd_t" | "aio_context_t" | "dev_t"
                | "nfds_t" | "loff_t" | "qid_t" | "idtype_t" | "time_t" | "timer_t" => {
                ...
                _ => {
                    if ty.ident == "Option" {
                        ...
                        match argstr.as_str() {
                            ...
                            | "Vec < c_ulong >" | "Vec < c_uint >" | "Vec < gid_t >"
                            | "timezone" | "mq_attr" | "siginfo_t" | "sigset_t" | "iovec"
                            | "rlimit64" | "fd_set" | "sockaddr" | "sigaction" | "timeval"
                            | "itimerval" | "stack_t" | "timer_t" | "time_t" | "sigevent"
                            | "itimerspec" | "utimbuf" | "rusage" => {
                                (quote_spanned!(span => InspectResult<#ty>), true)
                            }
                            "[timespec; 2]" | "[timeval; 2]" | "[timespec ; 2]"
                            | "[timeval ; 2]" => {
                        ...
                        match arg.as_str() {
                            "c_int" | "u8" | "CString" | "epoll_event" | "futex_waitv"
                            | "c_ulong" | "linux_dirent" | "io_event" | "linux_dirent64"
                            | "gid_t" | "AddressType" | "kexec_segment" | "c_uchar" | "u64"
                            | "mount_attr" | "pollfd" | "iovec" | "riscv_hwprobe" | "mmsghdr"
                            | "sembuf" => (quote_spanned!(span => InspectResult<#ty>), true),
